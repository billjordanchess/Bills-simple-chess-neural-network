...
char lines[100][80];
move_data book_list[MOVE_STACK];
int total;
int verbose = 0;

void LoadBook();
void InitBookboard();
int RandomBook(const int x);
...
int Book()
{
	...
	for (int x = 0; x < total; x++)
	{
		...
		for (y = 0; y < hply * 5; y += 5)
		{
			...
			book_board[dest] = book_board[start];
			book_color[dest] = book_color[start];
			book_board[start] = 6;
			book_color[start] = 6;
		}
		match = 1;
		for (int z = 0; z < 64; z++)
		{
			if ((board[z] != book_board[z]) || (color[z] != book_color[z]))
			{
				match = 0; break;
			}
		}
...
{
	int c = 0;
	strcpy_s(lines[c++], "c2c4 c7c5 b1c3 b8c6 g1f3 g8f6 d2d4 c5d4 ");
	strcpy_s(lines[c++], "c2c4 e7e5 b1c3 g8f6 g2g3 d7d5 c4d5 f6d5 ");
	...
	strcpy_s(lines[c++], "e2e4 e7e5 g1f3 b8c6 c2c3 g8f6 d2d4 e5d4 ");
	...
	strcpy_s(lines[c++], "e2e4 e7e5 g1f3 b8c6 f1b5 c6d4 f3d4 e5d4 ");
	strcpy_s(lines[c++], "e2e4 e7e5 g1f3 b8c6 f1b5 f7f5 b1c3 f5e4 ");
	strcpy_s(lines[c++], "e2e4 e7e5 g1f3 b8c6 f1b5 f7f5 d2d3 f5e4 ");
	...
	strcpy_s(lines[c++], "e2e4 d7d5 e4d5 d8d5 b1c3 d5d6 d2d4 g8f6 ");
	strcpy_s(lines[c++], "e2e4 d7d5 e4d5 g8f6 g1f3 c8g4 d2d4 d8d5 ");
	strcpy_s(lines[c++], "d2d4 d7d5 b1c3 g8f6 c1g5 b8d7 f2f3 c7c6 ");
	strcpy_s(lines[c++], "d2d4 d7d5 b1c3 g8f6 c1f4 c7c6 e2e3 c8f5 ");
	strcpy_s(lines[c++], "d2d4 d7d5 c2c4 e7e6 b1c3 g8f6 c1g5 f8e7 ");
	strcpy_s(lines[c++], "d2d4 d7d5 c2c4 e7e6 b1c3 g8f6 c1g5 b8d7 ");
	strcpy_s(lines[c++], "d2d4 d7d5 c2c4 e7e6 b1c3 g8f6 c1g5 c7c6 ");
	...
	strcpy_s(lines[c++], "d2d4 g8f6 c2c4 c7c5 d4d5 e7e5 b1c3 f8e7 ");
	...
	strcpy_s(lines[c++], "d2d4 f7f5 e2e4 f5e4 b1c3 g8f6 c1g5 e7e6 ");
...
void InitBookboard()
{
	for (int x = 0; x < 64; x++)
	{
		book_board[x] = init_board[x];
		book_color[x] = init_color[x];
	}
}
