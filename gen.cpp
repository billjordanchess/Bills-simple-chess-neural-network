...
GenEp looks at the last move played and sees if it is a float pawn move.
...
void GenEp()
{
	...
	if(board[ep] == 0 && abs(GetHistoryStart(hply - 1) - ep) == 16)
	{
		...
		{
			AddCapture(ep-1,ep+ForwardSquare[side],px[P]);				
		}
...
GenPawn generates single and float pawn moves and pawn 
captures for a pawn. 
...
{
	int k = 0;
	int sq = king_moves[kingloc[side]][k++];
	...
	while(sq > -1)
	{
		...
		sq = king_moves[kingloc[side]][k++];
	}
	for(int x = 0;x < 64;x++)
	{
		if(color[x] == side && board[x]==P)
		{
			if(board[x+ForwardSquare[side]] == EMPTY)
			{
				return true;
